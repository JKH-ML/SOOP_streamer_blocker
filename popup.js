document.addEventListener('DOMContentLoaded', function() {
    ...
    masterToggle.addEventListener('change', async function() {
        ...
        if (!isEnabled) {
            ...
            const result = await chrome.storage.sync.get(['streamerBlockEnabled', 'tagBlockEnabled']);
        ...
        try {
            ...
            if (tab.url && tab.url.includes('sooplive.co.kr')) {
                ...
                if (isEnabled) {
                    ...
                    pageStatusEl.innerHTML = '<span class="disabled-indicator"></span>차단 기능 비활성화됨';
                }
            } else {
                pageStatusEl.innerHTML = '<span class="disabled-indicator"></span>SoopLive 페이지가 아님';
            }
        } catch (error) {
